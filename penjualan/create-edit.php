...
if ($isEdit) {
  ...
  if ($data['is_locked'] == 1 && $data['locked_by'] !== $client_name) {
    $_SESSION['error'] = "Transaksi sedang digunakan oleh user lain: <b>{$data['locked_by']}</b>.";
    header("Location: index.php");
    exit;
  }
    ...
    <div class="card-body pt-0 bg-light">
      <form action="<?= $isEdit ? 'update.php' : 'store.php' ?>" method="POST" class="mt-4">
        ...
        <div class="d-flex justify-content-end mt-5">
          ...
          <a href="cancel.php<?= $isEdit ? '?id=' . $data['kd_trans'] : '' ?>" class="btn btn-secondary ms-2">Batal</a>
        </div>
      </form>
    </div>
  </div>
